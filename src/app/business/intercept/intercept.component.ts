...
export class InterceptComponent implements OnInit {
  ...
  public addCar: Intercept = new Intercept();
  public revampDialog: boolean; // 修改弹窗
  public detailsDialog: boolean; // 详情弹窗
  public searchField: string;
  public cars: Intercept[]; // 整个table数据
  public cols: any[]; // 表头
  public car1: any; // 接收选中的值
  public selectedCars3: Intercept[]; // 多选接受变量
  ...
  public addsSave(): void {
    this.confirmationService.confirm({
      ...
      accept: () => {
        ...
        this.interceptService.addList().subscribe(
          (value) => {
            if (value.state) {
              setTimeout(() => {
  ...
  public revampSave(): void {
    this.confirmationService.confirm({
      ...
      accept: () => {
        this.globalService.eventSubject.next({display: true});
        this.interceptService.modifyList().subscribe(
          (value) => {
            if (value.state) {
              setTimeout(() => {
                this.globalService.eventSubject.next({display: false});
  ...
  public deleteFirm(): void {
    if (this.selectedCars3 === undefined || this.selectedCars3.length === 0) {
      ...
      this.confirmationService.confirm({
        ...
        accept: () => {
          this.globalService.eventSubject.next({display: true});
          this.interceptService.deleteList().subscribe(
            (value) => {
              if (value.state) {
                setTimeout(() => {
                  this.globalService.eventSubject.next({display: false});
  ...
  public searchClick(): void {
    this.globalService.eventSubject.next({display: true});
    this.interceptService.searchList({'name': '文君', 'age': '18'}).subscribe(
      (value) => {
        console.log(value);
        /*setTimeout(() => {
          this.globalService.eventSubject.next({display: false});
